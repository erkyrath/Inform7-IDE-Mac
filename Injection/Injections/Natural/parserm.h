...
[ KeyboardPrimitive__  a_buffer a_table i;
...
[ KeyboardPrimitive__  a_buffer a_table done ix;
    if (gg_commandstr ~= 0 && gg_command_reading ~= false) {
        ! get_line_stream
...
[ SpliceSnippet snip t i w nextw at endsnippet newlen;
  !print "[SpliceSnippet snip=", snip, ", t=", t, "]^";
  if (snip==0) ">--> You tried to replace or cut an empty snippet.";
  w = snip/100;
  nextw = w + snip%100;
  at = WordAddress(w) - buffer;
  if (nextw <= WordCount())
      endsnippet = 100*nextw + (WordCount() - nextw + 1);
  !print "At is ", at, ", nextw is ", nextw, ", and endsnippet is ", endsnippet, "^";
  buffer2-->0 = 120;
  newlen = PrintToBuffer(buffer2, 120, SpliceSnippet__Cat, t, endsnippet);
...
[ SpliceSnippet__Cat t endsnippet;
  switch (metaclass(t)) {
...
[ Parser__parse  results   syntax line num_lines line_address i j k
  ...
  .BeginCommand;
    ...
    if (verb_word == 0 || ((verb_word->#dict_par1) & 1) == 0) {
        ...
        #endif;
            results-->0 = ##Go;
            #ifndef NI_BUILD_COUNT; action_to_be = ##Go; #endif;
            results-->1 = 1;
            results-->2 = l;
            jump LookForMore;
        }
  ...
  .VerbAccepted;
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        for (pcount=1 : : pcount++) {
            ...
            if (token ~= ENDIT_TOKEN) {
                scope_reason = PARSING_REASON;
                #ifndef NI_BUILD_COUNT; parser_inflection = name; #endif;
                AnalyseToken(token);
...
#ifndef NI_BUILD_COUNT;
[ ParseToken given_ttype given_tdata token_n token x y;
    x = lookahead; lookahead = NOUN_TOKEN;
    y = ParseToken__(given_ttype,given_tdata,token_n,token);
...
[ ParseToken__
    ...
    token_filter = 0;
    #ifdef NI_BUILD_COUNT; parser_inflection = name; #endif;
...
!          (token_filter==0 || UserFilter(n)==1)) { good_ones++; last=n; }
    ...
    if (indef_mode == 1 && indef_type & PLURAL_BIT ~= 0) {
        ...
        #ifndef NI_BUILD_COUNT; sovert = -1; #endif;
...
[ BestGuess  earliest its_score best i;
    ...
    #ifndef NI_BUILD_COUNT; bestguess_score = best; #endif;
    return i;
];
...
[ SearchScope domain1 domain2 context i;
    ...
    if (context == MULTIINSIDE_TOKEN && advance_warning ~= -1) {
        ...
        #ifdef NI_BUILD_COUNT; I7_ScopeParts(advance_warning); #endif;
    }
    else {
...
Object  InformLibrary "(Inform Library)"
  with  play [ i j k l;
            ...
            #ifdef NI_BUILD_COUNT; act_requester = nothing; #endif;
            ...
            #ifdef FIX_RNG;
            #ifdef TARGET_ZCODE; @random 10000 -> i; #endif;
            #ifdef TARGET_GLULX; @random 10000 i; #endif;
            i = -i-2000;
            print "[Random number generator seed is ", i, "]^";
            #ifdef TARGET_ZCODE; @random i -> i; #endif;
            #ifdef TARGET_GLULX; @setrandom i; #endif;
            #endif; ! FIX_RNG
        ...
        begin_action [ a n s source   sa sn ss rv;
...
[ AfterGameOver i;
  ...
  .RRQL;
    ...
    if (i == RESTORE__WD) {
    	action = ##Restore; noun = 0; second = 0; actor = player;
        RestoreSub();
        jump RRQPL;
    }
...
[ NotifyTheScore;
    ...
    #Endif; ! TARGET_GLULX
    #ifdef NI_BUILD_COUNT; I7_DivideParagraph(); #endif;
...
#ifdef NI_BUILD_COUNT;
    if (o == player) { 
   	    if (indef_mode == NULL && I7_caps_mode) print "You";
...
[ CDefart o i;
    ...
    #Endif;
    i = indef_mode; indef_mode = false;
    if ((o ofclass Object) && (o has proper || o == player)) {
    	indef_mode = NULL;
    	#Ifdef NI_BUILD_COUNT; I7_caps_mode = true; #Endif;
    	print (PSN__) o;
    	indef_mode = i;
