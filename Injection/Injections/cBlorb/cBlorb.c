...
void frontispiece_chunk(int n) {
    begin_chunk("Fspc", 0, NULL);
    four_word(CHUNK, n);
    end_chunk();
}
...
void executable_chunk(char *fn) {
	if (fn[strlen(fn)-1] == 'x') executable_type = "GLUL";
...
#define MAX_TABLES 250
int table_line_start[MAX_TABLES];
int table_line_end[MAX_TABLES];
int no_tables, table_active;

#define MAX_HEADINGS 250
int heading_to_segment[MAX_HEADINGS];
int heading_line[MAX_HEADINGS];
int heading_level[MAX_HEADINGS];
int heading_has_content[MAX_HEADINGS];
int segment_begins_at[MAX_HEADINGS];
int no_headings;
int no_segments;
int current_html_source_segment = 0;

int source_written = FALSE;

void scan_source_line(char *line, int line_count) {
	char fword[32]; int i, lv = 0;
	for (i=0; (line[i]) && (line[i] != ' ') && (line[i] != '\t') && (i<30); i++)
		fword[i] = tolower(line[i]);
	fword[i] = 0;
	...
	if ((fword[0]) && (lv == 0) && (no_headings > 0))
		heading_has_content[no_headings-1] = TRUE;
	if (strcmp(fword, "table") == 0) {
		table_line_start[no_tables] = line_count;
		table_line_end[no_tables++] = 1000000;
		table_active = TRUE;
		return;
	}
	if ((table_active) && (line[0] == 0)) {
		table_line_end[no_tables-1] = line_count;
		table_active = FALSE;
		return;
	}
	if (lv == 0) return;
	heading_level[no_headings] = lv;
	heading_line[no_headings] = line_count;
	heading_has_content[no_headings] = FALSE;
	if ((no_headings>0) && (heading_has_content[no_headings-1] == FALSE)) {
		heading_to_segment[no_headings++] = no_segments-1;
	} else {
		segment_begins_at[no_segments] = line_count;
		heading_to_segment[no_headings++] = no_segments++;
	}
}

int ul_begun = FALSE, quoted_matter = FALSE, quoted_comment = FALSE;
int footnote_count;
char *quoted_colour = NULL;

void write_source_line(char *line, int line_count, FILE *write_to, int noting_mode) {
	int i, j, embolden = FALSE, tabulate = FALSE, underline = FALSE;
	if ((current_html_source_segment == -1) && (noting_mode == FALSE)) {
		if ((no_segments > 0) && (line_count == segment_begins_at[0]-1) &&
			(line[0] == 0)) return;
		if ((no_segments == 0) || (line_count < segment_begins_at[0]))
			goto LineOut;
		for (i=0; i<no_headings; i++) {
			if (heading_line[i] == line_count) {
				if (ul_begun == FALSE) {
					ul_begun = TRUE;
					fprintf(write_to, "<ul>");
				}
				fprintf(write_to, "<li>");
				for (j=0; j<heading_level[i]; j++)
					fprintf(write_to, "- ");
				fprintf(write_to, "<a href=source_%d.html>%s</a></li>\n",
					heading_to_segment[i], line);
			}
		}
		return;
	}
	if (line_count < segment_begins_at[current_html_source_segment]) return;
	if (current_html_source_segment+1 < no_segments)
		if (line_count >= segment_begins_at[current_html_source_segment+1]) return;
	LineOut:

	if (noting_mode) {
		for (i=0; line[i]; i++) {
			if ((line[i] == '[') && (line[i+1] == '*')) {
				fprintf(write_to, "<A name=\"note%d\"><font color=#404040>[%d]. ",
					footnote_count,
					footnote_count);
				footnote_count++;
				i+=2;
				while ((line[i]) && (line[i] != ']')) {
					fprintf(write_to, "%c", line[i++]);
				}
				fprintf(write_to, "</font><p>\n");
			}
		}
		return;
	}
	...
	for (i=0; i<no_tables; i++) {
		if (table_line_start[i] == line_count) {
			embolden = TRUE;
		} else
		if ((line_count >= table_line_start[i]+1) &&
			(line_count < table_line_end[i])) {
			tabulate = TRUE;
			if (line_count == table_line_start[i]+1) {
				underline = TRUE;
				fprintf(write_to, "<table>");
			}
		} else
		if (table_line_end[i] == line_count) {
			fprintf(write_to, "</table>");
		}
	}

	if ((line_count == 1) ||
		((current_html_source_segment >= 0) &&
			(line_count == segment_begins_at[current_html_source_segment])))
			embolden = TRUE;
	for (i=0; i<no_headings; i++)
		if (heading_line[i] == line_count)
			embolden = TRUE;

	if (tabulate) fprintf(write_to, "<tr><td halign=\"left\" valign=\"top\">");
	if (underline) fprintf(write_to, "<u>");

	if (embolden) fprintf(write_to, "<b>");
	...
	for (i=j; line[i]; i++) {
		...
		switch (line[i]) {
			case '\t':
				while (line[i+1] == '\t') i++;
				if (underline) fprintf(write_to, "</u>");
				if (tabulate) fprintf(write_to, "&nbsp;&nbsp;</td><td halign=\"left\" valign=\"top\">");
				if (underline) fprintf(write_to, "<u>");
				fprintf(write_to, " ");
				break;
			case '"':
				change_to = quoted_matter;
				if (quoted_matter) {
					fprintf(write_to, "</font>");
					quoted_colour = NULL;
					change_to = FALSE;
				}
				fprintf(write_to, "&quot;");
				if ((quoted_matter == FALSE) && (quoted_comment == FALSE)) {
					quoted_colour = "000080";
					fprintf(write_to, "<font color=#%s>", quoted_colour);
					change_to = TRUE;
				}
				quoted_matter = change_to;
				break;
			case '[':
				if (line[i+1] == '*') {
					fprintf(write_to,
						"<font color=#404040><sup><a href=\"#note%d\">[%d]</a></sup></font>",
						footnote_count,
						footnote_count);
					footnote_count++;
				...
				if (quoted_matter == FALSE) {
					quoted_colour = "404040";
					fprintf(write_to, "<font color=#%s>", quoted_colour);
					quoted_comment = TRUE;
				}
				fprintf(write_to, "["); break;				
			case ']':
				if (quoted_comment) {
					quoted_colour = NULL;
					fprintf(write_to, "</font>");
					quoted_comment = FALSE;
				}
				fprintf(write_to, "]"); break;				
			case '<': fprintf(write_to, "&lt;"); break;
			case '>': fprintf(write_to, "&gt;"); break;
			case '&': fprintf(write_to, "&amp;"); break;
			default: fprintf(write_to, "%c", line[i]); break;
		}
	}
	if (embolden) fprintf(write_to, "</b>");
	if (underline) fprintf(write_to, "</u>");
	if (tabulate) fprintf(write_to, "</td></tr>\n");
	else fprintf(write_to, "\n<br>\n");
	if (quoted_colour) fprintf(write_to, "</font>");
	if (quoted_colour) fprintf(write_to, "<font color=#%s>", quoted_colour);
}

void write_html_source(FILE *write_to, int noting_mode) {
	FILE *SOURCE; int i, c, lc=1; char line[10240];
	source_written = TRUE;
	ul_begun = FALSE; quoted_matter = FALSE; quoted_comment = FALSE;
	quoted_colour = NULL;
	if (noting_mode) {
		if (footnote_count == 1) return;
		if (footnote_count == 2) fprintf(write_to, "<p><i>Note</i><p>\n");
		if (footnote_count > 2) fprintf(write_to, "<p><i>Notes</i><p>\n");
	}
	footnote_count = 1;
	...
	while (c != EOF) {
		...
		if ((c == EOF) || (c == '\x0a') || (c == '\x0d') || (i>=10239)) {
			line[i] = 0; write_source_line(line, lc++, write_to, noting_mode); i=0;
		} else line[i++] = c;
	}
	if (i>0) { line[i] = 0; write_source_line(line, lc++, write_to, noting_mode); }		
	fclose(SOURCE);
	if ((current_html_source_segment == -1) && (noting_mode == FALSE))
		fprintf(write_to, "</ul>");
}

int html_source_index(void) {
	FILE *SOURCE; int i, c, lc=1, minhl; char line[10240];
	no_segments = 0;
	no_headings = 0;
	no_tables = 0;
	table_active = FALSE;
	...
	while (c != EOF) {
		c = fgetc(SOURCE);
		if ((c == EOF) || (c == '\x0a') || (c == '\x0d') || (i>=10239)) {
			line[i] = 0; scan_source_line(line, lc++); i=0;
		} else line[i++] = c;
	}
	if (i>0) { line[i] = 0; scan_source_line(line, lc++); }		
	fclose(SOURCE);
	minhl = 10;
	for (i=0; i<no_headings; i++)
		if (heading_level[i] < minhl)
			minhl = heading_level[i];
	for (i=0; i<no_headings; i++)
		heading_level[i] -= minhl;
	return no_segments;
}
...
void copy_variable_to(char *var, FILE *COPYTO) {
	...
	if (strcmp(var, "SOURCE") == 0) {
		write_html_source(COPYTO, FALSE);
		return;
	}
	if (strcmp(var, "SOURCENOTES") == 0) {
		write_html_source(COPYTO, TRUE);
		return;
	}
	if (strcmp(var, "SOURCELINKS") == 0) {
		fprintf(COPYTO, "<a href=\"index.html\">Home page</a><br>");
		if (no_segments > 0) {
			fprintf(COPYTO, "<br><a href=\"source.html\">Contents</a><br>");
			if (current_html_source_segment > 0)
				fprintf(COPYTO, "<a href=\"source_%d.html\">Previous</a><br>",
					current_html_source_segment-1);
			if ((current_html_source_segment >= 0)
				&& (current_html_source_segment < no_segments-1))
				fprintf(COPYTO, "<a href=\"source_%d.html\">Next</a><br>",
					current_html_source_segment+1);
		}
		fprintf(COPYTO, "<br><a href=\"source.txt\">Complete text</a>");
		return;
	}
	if (strcmp(var, "COVER") == 0) {
		if (cover_exists == FALSE) return;
		if (cover_jpeg)
		fprintf(COPYTO, "<a href=\"Cover.jpg\"><img src=\"Small Cover.jpg\" border=\"1\"></a>");
		else
		fprintf(COPYTO, "<a href=\"Cover.png\"><img src=\"Small Cover.png\" border=\"1\"></a>");
		return;
	}
	if (strcmp(var, "DOWNLOAD") == 0) {
		download_link(COPYTO, "Story File", target_leafname, "Blorb");
		fprintf(COPYTO, "<p>");
		return;
	}
	if (strcmp(var, "AUXILIARY") == 0) {
		...
		if (source_written) {
			download_link(COPYTO, "Source Text", "source.html", "link");
			fprintf(COPYTO, "<p>");
		}
		return;
	}
...
void read_skein_line(char *line, int pass) {
	char node_id[10240], junk[10240], junk2[10240]; int i;
	...
	if (pass == 1) {
		if (sscanf(line, "%[^<]<item nodeId=\"%[^\"]\"", junk, node_id) == 2) {
			...
			current_skein_node->terminal = FALSE;
		}
...
void source(char *from, char *to) {
	FILE *SOURCE, *SRC; int i, c; char line[10240];
	...
	SRC = fopen(to, "w");
	if (SRC == NULL) {
		error("unable to open destination for source text");
		return;
	}
	i=0; c = ' ';
	while (c != EOF) {
		c = fgetc(SOURCE);
		if ((c == EOF) || (c == '\x0a') || (c == '\x0d') || (i>=10239)) {
			line[i] = 0; fprintf(SRC, "%s\n", line); i=0;
		} else line[i++] = c;
	}
	if (i>0) { line[i] = 0; fprintf(SRC, "%s\n", line); }		
	fclose(SOURCE);
	fclose(SRC);
}
...
void interpret(char *command) {
	...
	if (sscanf(command, "source \"%[^\"]\"", sv) == 1) {
		strcpy(source_filename, sv);
		return;
    }
	if (sscanf(command, "source to \"%[^\"]\"", sv) == 1) {
		source(source_filename, sv);
        return;
    }
	if (sscanf(command, "source from \"%[^\"]\" to \"%[^\"]\"", sv, sv2) == 2) {
		char url[2048];
		int sc;
		sc = html_source_index();
		current_html_source_segment = -1;
		sprintf(url, "%s%csource.html", sv2, SEP_CHAR);
		web_copy(sv, url);
		for (current_html_source_segment=0; current_html_source_segment<sc;
			current_html_source_segment++) {
			sprintf(url, "%s%csource_%d.html", sv2, SEP_CHAR,
				current_html_source_segment);
			web_copy(sv, url);
		}
        return;
	...
	if (sscanf(command, "cover \"%[^\"]\"", sv) == 1) {
		cover_exists = TRUE;
		cover_jpeg = TRUE;
		set_variable_to("BIGCOVER", sv);
		if ((sv[strlen(sv)-3] == 'p') ||
			(sv[strlen(sv)-3] == 'P')) cover_jpeg = FALSE;
...
/*    if ($command =~ /^\s*resolution\s+(\d*)x(\d*)\s*(.*)$/m)
    ...
    if (sscanf(command, "destination \"%[^\"]\"", sv) == 1) {
    	int k = strlen(sv)-1;
    	while ((k>=0) && (sv[k] != SEP_CHAR)) k--; k++;
    	strcpy(target_leafname, sv+k);
...
int main(int argc, char *argv[]) {
	...
	strcpy(target_leafname, "story.blorb");
